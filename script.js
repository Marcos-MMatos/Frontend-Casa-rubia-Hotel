...
function initRoomsPage() {
  ...
  async function updateAvailability() {
    ...
    try {
      ...
      const response = await fetch(
        `https://casa-rubia-hotel-production.up.railway.app/api/reservations?${params.toString()}`
      );
  ...
  function openReservationModal(roomId) {
    ...
    if (
      ...
      alert(
        'Por favor seleccione una fecha de entrada y salida válida antes de reservar.'
      );
      return;
    }
    ...
    document.getElementById('modalRoomPrice').textContent = formatCOP(
      room.price
    );
  ...
  reservationForm.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch(
        'https://casa-rubia-hotel-production.up.railway.app/api/reservations',
        {
          method: 'POST',
      ...
      await response.json();
      alert(
        '¡Gracias! Su solicitud de reserva ha sido registrada. Nos pondremos en contacto a la brevedad.'
      );
      closeModal();
      updateAvailability();
    } catch (err) {
      ...
      alert(
        'Lo sentimos, ocurrió un error al registrar la reserva. Por favor inténtelo de nuevo más tarde.'
      );
    }
  });
...
function initContactForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const body = encodeURIComponent(
      `Nombre: ${name}%0ACorreo: ${email}%0ATeléfono: ${phone}%0A%0AMensaje:%0A${message}`
    );
    ...
    alert(
      'Gracias por contactarnos. Se abrirá su cliente de correo para enviar el mensaje.'
    );
    form.reset();
  });
}
